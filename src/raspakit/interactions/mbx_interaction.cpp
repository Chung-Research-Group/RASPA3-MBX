...
// Use to calculate energies for the given fixed positions of framework, adsorbate molecules, and selected trial
// molecule for the MC move. The energy of the system can be calculated by either including or excluding the trial
// molecule.
RunningEnergy Interactions::computeMBXEnergy(const System& system, const std::vector<Component>& components,
                                             ...
                                             std::vector<double>* mbxEnergyLog) noexcept
{
  /***
   * Setup MBX System object.
   ***/
  ...
  if (!selectedMoleculeAtoms.empty())
  {
    ...
    if (excludeSelectedMolecule && selectedMoleculeId >= system.numberOfMoleculesPerComponent[selectedComponent])
    {
  ...
  for (size_t i = 0; i < components.size(); ++i)
  {
    ...
    for (size_t j = 0; j < system.numberOfMoleculesPerComponent[i]; ++j)
    {
      // for (const Atom &atom : selectedMoleculeAtoms)
      // {
      //     if ( (j == static_cast<size_t>(atom.moleculeId)) && (selectedComponent ==
      //     static_cast<size_t>(atom.componentId)) )
      //     {
      //         keepMolecule = false;
      //         break;
      //     }
    ...
    std::span<const Atom> moleculeAtoms, std::span<const Atom> newatoms, std::span<const Atom> oldatoms) noexcept
{
  ...
  RunningEnergy oldEnergy = Interactions::computeMBXEnergy(
      system, components, simulationBox, framework, selectedComponent, frameworkAtoms, moleculeAtoms, oldatoms, true);

  // Energy of the configuration after the trial MC move
  RunningEnergy newEnergy = Interactions::computeMBXEnergy(
      system, components, simulationBox, framework, selectedComponent, frameworkAtoms, moleculeAtoms, newatoms, true);
