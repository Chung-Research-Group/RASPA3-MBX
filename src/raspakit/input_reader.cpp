...
InputReader::InputReader(const std::string inputFile)
{
  ...
  if (!input)
  {
    throw std::runtime_error(std::format("[Input reader]: Failed to open file '{}'\n", inputFile));
  }
...
void InputReader::parseMolecularSimulations(const nlohmann::basic_json<nlohmann::raspa_map>& parsed_data)
{
  ...
  if (parsed_data.contains("Systems"))
  {
    for (std::size_t systemId = 0; auto& [key, value] : parsed_data["Systems"].items())
    {
      ...
      if (value.contains("UseMBX"))
      {
        ...
        {
          throw std::runtime_error("[Input reader]: 'UseMBX' must be a boolean");
        }
      ...
      {
        throw std::runtime_error("[Input reader]: 'MBXSettingsFile' must be a string path");
      }
      ...
      if (useMBXCalculator)
      {
#ifndef BUILD_MBX
        throw std::runtime_error(
            "[Input reader]: 'UseMBX' is true, but this RASPA binary was built without BUILD_MBX support; "
            "reconfigure with -DBUILD_MBX=ON and provide MBX_INCLUDE_DIR, MBX_CONFIG_INCLUDE_DIR, and "
            "MBX_LIBRARY (or make MBX discoverable through CMAKE_PREFIX_PATH)");
#else
        if (!value.contains("MBXSettingsFile") || !value["MBXSettingsFile"].is_string())
        {
          throw std::runtime_error(
        ...
        {
          throw std::runtime_error("[Input reader]: 'MBXSettingsFile' must not be empty");
        }
        if (settingsPath.is_relative()) settingsPath = inputDirectory / settingsPath;
        ...
        {
          throw std::runtime_error(
              std::format("[Input reader]: MBX settings file '{}' is unreadable or empty", settingsPath.string()));
        }

        const std::filesystem::path canonicalSettings = std::filesystem::canonical(settingsPath, settingsError);
        mbxFilePath = settingsError ? std::filesystem::absolute(settingsPath).lexically_normal().string()
  ...
  for (std::size_t systemId = 0uz; systemId < systems.size(); ++systemId)
  {
