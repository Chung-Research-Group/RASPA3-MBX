      ...
      interpolationGrids(forceField.pseudoAtoms.size() + 1, std::nullopt)
{
  ...
  if (useMBXCalculator.has_value())
  {
    if (useMBXCalculator.value())
    {
      ...
      std::cerr << "type, N, total, hg_VDW, hg_tail, mbx_tot, E2b, E3b, E4b, Edisp, Eelec_perm, Eelec_ind, E_diff, Pacc\n"; // Header for MBX energy log
    }
    else{
      std::cerr << "type, N, total, hg_VDW, gg_VDW, tail, hg_Charge, gg_Charge, E_ewald, E_diff, Pacc\n"; // Header for FF energy log
    }
  }
