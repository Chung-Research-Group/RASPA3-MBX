...
TEST(mbx_energy_log, Test_component_rows_use_component_loading_and_volume_rows_use_total_loading)
{
  ...
  System system =
      System(forceField, std::nullopt, false, 300.0, 1e4, 1.0, {framework}, components, {}, {1, 1}, 5);
  system.useMBX = true;
  ...
  const std::string componentRow = testing::internal::GetCapturedStderr();
  EXPECT_NE(std::string::npos, componentRow.find("\ntranslation,1,1,"));

  testing::internal::CaptureStderr();
  system.writeAcceptedEnergyLog("volume", std::numeric_limits<std::size_t>::max(), totalEnergy, mbxTerms, 0.0, 1.0);
  const std::string volumeRow = testing::internal::GetCapturedStderr();
  EXPECT_TRUE(volumeRow.starts_with("volume,all,2,"));
}
